 ...
 */
final class ModelSchemaIntegrationService
{
    ...
    public function processYamlFile(string $filePath): array
    {
        ...
        if (isset($yamlData['fields']) && is_array($yamlData['fields'])) {
            foreach ($yamlData['fields'] as $fieldName => $fieldConfig) {
                $processed = $this->processField($fieldConfig);
    ...
    public function getIntegrationStatistics(): array
    {
        ...
        $geometricTypes = array_filter($allTypes, function ($type): bool {
     ...
     */
    private function extractModelSchemaMetadata(array $yamlData): array
    {
        $metadata = [
